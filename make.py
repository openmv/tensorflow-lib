...
def generate(target, __folder__, cpus, builddir, bindir, compile_flags):
    ...
    if os.system("cd " + os.path.join(builddir, target) + " && make -j" + str(cpus) + 
        " && arm-none-eabi-gcc -o libtf-mobilenet.o -c libtf-mobilenet.c" +
    ...
    with open(os.path.join(bindir, target, "README.txt"), 'w') as file:
        file.write("Compiled with -mthumb -nostartfiles -fdata-sections -ffunction-sections " + compile_flags + "\n")
...
def build_target(target, __folder__, cpus, builddir, bindir):
    ...
    if (target == "OPENMV1") or (target == "OPENMV2") or (target == "cortex-m4"):
        generate(target, __folder__, cpus, builddir, bindir, "-mcpu=cortex-m4 -mtune=cortex-m4 -mfpu=fpv4-sp-d16 -mfloat-abi=hard")
    elif (target == "OPENMV3") or (target == "OPENMV4") or (target == "cortex-m7"):
        generate(target, __folder__, cpus, builddir, bindir, "-mcpu=cortex-m7 -mtune=cortex-m7 -mfpu=fpv5-sp-d16 -mfloat-abi=hard")
    else:
